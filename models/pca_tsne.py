...
from models.plot import plot_pls_results, plot_similarity_matrix, save_loading_scores, create_cumulative_variance_plot, create_individual_variance_plot
...
class DimensionalityReducer:
    ...
    def perform_regression(self, data, target):
        """Perform PLS analysis and return the reduced data."""
        target = target.to_numpy().reshape(-1, 1).ravel()
        ...
        for n_components in components_range:
            ...
            for train_index, test_index in kf.split(data):
                ...
                with warnings.catch_warnings():
                    ...
                    trainer.train_and_evaluate(X_train, y_train)
            # Assuming the best model is stored in trainer.best_model
                ...
                for loo_train_index, loo_test_index in loo.split(X_train):
                    trainer.best_model.fit(X_train[loo_train_index], y_train[loo_train_index])
                    y_pred_loo[loo_test_index] = trainer.best_model.predict(X_train[loo_test_index])
        ...
        trainer.best_model.fit(data, target)
        reduced_data = trainer.best_model.transform(data)
        return reduced_data
    ...
    def fit_transform(self, data, log):
        ...
        #variance explained and loading scores
        ...
        reduced_data = self.perform_regression(pca_transformed, log)
